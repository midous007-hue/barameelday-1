...
function rewardSound(){
  /* BIG GOAL / JACKPOT FANFARE — original retro arcade-style hit, ~2.45s.
     Shorter, louder and more decisive: rising goal call -> coin burst -> bright victory chord. */
  const c=audio();
  master.gain.cancelScheduledValues(c.currentTime);
  master.gain.setValueAtTime(1.0,c.currentTime);
  const rise=[392,494,587,698,784,988,1175,1397,1568,1760];
  rise.forEach((f,i)=>{
    const t=i*.075;
    tone(f,.105,'square',.86,t,i<rise.length-1?120:220);
    tone(f*2,.075,'triangle',.18,t+.012,i===rise.length-1?300:0);
  });
  // rapid reward / coin cascade
  [1319,1568,1760,2093,2349,2637,3136,3520,3951].forEach((f,i)=>{
    const t=.82+i*.055;
    tone(f,.10,'square',.76,t,i===8?320:100);
  });
  // unmistakable final victory chord + octave shimmer
  [1047,1319,1568,2093,2637].forEach((f,i)=>{
    tone(f,.92,'square',.82,1.42+i*.012,i===4?260:0);
    tone(f/2,.84,'triangle',.30,1.42+i*.012,0);
  });
  [3136,3520,3951].forEach((f,i)=>tone(f,.42,'sine',.22,1.56+i*.035,120));
  noise(.10,.30,.02);
  noise(.11,.28,.80);
  noise(.18,.34,1.42);
  master.gain.setValueAtTime(1.0,c.currentTime+1.65);
  master.gain.exponentialRampToValueAtTime(.72,c.currentTime+2.45);
}
...
function warmCurrentFlow(){
  ...
  if(p==='screen06.html')preload('./assets/screen07.png');
}
...
if('serviceWorker' in navigator && location.protocol==='https:'){navigator.serviceWorker.register('./sw.js?v=16').catch(()=>{});}
