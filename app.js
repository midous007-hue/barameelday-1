...
function rewardSound(){
  const seq=[523,659,784,1047,1319,1568,1760];
  seq.forEach((f,i)=>tone(f,.11,'square',.64,i*.075,i===seq.length-1?160:0));
  tone(1047,.24,'triangle',.28,.56,120);
  tone(1319,.30,'triangle',.24,.62,180);
  noise(.10,.14,.02);
}
...
function warmCurrentFlow(){
  ...
  if(p==='screen05.html')preload('./assets/screen06-base.png');
...
if('serviceWorker' in navigator && location.protocol==='https:'){navigator.serviceWorker.register('./sw.js?v=6').catch(()=>{});}
