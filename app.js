...
function preload(src){if(!src)return;const im=new Image();im.decoding='async';im.src=src;return im}
function preloadNext(srcs=[]){srcs.filter(Boolean).forEach(preload)}
function preloadMany(srcs=[]){srcs.filter(Boolean).forEach(preload)}
...
function warmCurrentFlow(){
  ...
  if(p==='screen02.html'){preloadMany(CHARACTERS.map(k=>`./assets/screen03-${k}.png`));preload(`./assets/screen03-${key}.png`);}
  if(p==='screen03.html')preload(`./assets/screen04-${key}.png`);
  if(p==='screen04.html')preload('./assets/screen05-bg.png');
}
...
if('serviceWorker' in navigator && location.protocol==='https:'){navigator.serviceWorker.register('./sw.js?v=5').catch(()=>{});}

window.BR={CHARACTERS,RUNNERS,selected,selectRunner,go,flash,toast,selectSound,confirmSound,scanSound,errorSound,backSound,preload,setArt,preloadNext,preloadMany};
