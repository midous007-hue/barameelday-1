...
function rewardSound(){
  /* Strong checkpoint reward: coin cascade + short arcade fanfare.
     Louder than navigation sounds, but still inside the same compressed audio chain. */
  const coins=[1047,1319,1568,1976,2349,2637,3136];
  coins.forEach((f,i)=>{
    tone(f,.085,'square',.76,i*.065,i<coins.length-1?90:180);
    tone(f*2,.055,'triangle',.22,i*.065+.012,120);
  });
  [784,988,1175,1568,2093].forEach((f,i)=>tone(f,.13,'square',.64,.48+i*.055,i===4?240:70));
  tone(1047,.38,'triangle',.34,.78,180);
  tone(1568,.44,'triangle',.30,.82,260);
  noise(.13,.18,.02);
  noise(.08,.12,.56);
}
...
if('serviceWorker' in navigator && location.protocol==='https:'){navigator.serviceWorker.register('./sw.js?v=9').catch(()=>{});}
