...
function rewardSound(){
  /* JACKPOT checkpoint reward — intentionally louder/longer than navigation sounds.
     Original retro coin-cascade/fanfare built from WebAudio; not a copy of a specific game. */
  const c=audio();
  master.gain.setValueAtTime(1.05,c.currentTime);
  const coins=[880,1047,1319,1568,1760,2093,2349,2637,3136];
  coins.forEach((f,i)=>{
    tone(f,.12,'square',.82,i*.075,i<coins.length-1?120:260);
    tone(f*1.5,.075,'triangle',.24,i*.075+.018,180);
  });
  /* bright jackpot climb */
  [784,988,1175,1397,1568,1760,2093,2637].forEach((f,i)=>
    tone(f,.16,'square',.72,.70+i*.07,i===7?320:80)
  );
  /* final victory chord */
  [1047,1319,1568,2093,2637].forEach((f,i)=>{
    tone(f,.52,'square',.62,1.30+i*.012,i===4?180:0);
    tone(f/2,.44,'triangle',.30,1.30+i*.012,0);
  });
  noise(.16,.24,.02);
  noise(.12,.18,.68);
  noise(.15,.20,1.30);
...
if('serviceWorker' in navigator && location.protocol==='https:'){navigator.serviceWorker.register('./sw.js?v=11').catch(()=>{});}
