...
function rewardSound(){
  /* BIG CHECKPOINT JACKPOT — original WebAudio fanfare, deliberately louder and more dramatic than navigation sounds. */
  const c=audio();
  master.gain.cancelScheduledValues(c.currentTime);
  master.gain.setValueAtTime(1.08,c.currentTime);
  const coins=[659,784,988,1175,1397,1568,1760,2093,2349,2637];
  coins.forEach((f,i)=>{
    tone(f,.13,'square',.88,i*.065,i<coins.length-1?150:320);
    tone(f*2,.075,'triangle',.20,i*.065+.012,180);
  });
  /* jackpot rise */
  [784,988,1175,1397,1568,1760,2093,2349,2637,3136].forEach((f,i)=>
    tone(f,.18,'square',.78,.68+i*.065,i===9?420:95)
  );
  /* heavy final win chord */
  [1047,1319,1568,2093,2637,3136].forEach((f,i)=>{
    tone(f,.62,'square',.70,1.34+i*.012,i===5?220:0);
    tone(f/2,.50,'triangle',.34,1.34+i*.012,0);
  });
  noise(.18,.30,.02);
  noise(.13,.22,.68);
  noise(.18,.26,1.34);
  master.gain.setValueAtTime(1.08,c.currentTime+1.85);
  master.gain.exponentialRampToValueAtTime(.88,c.currentTime+2.65);
}
...
if('serviceWorker' in navigator && location.protocol==='https:'){navigator.serviceWorker.register('./sw.js?v=12').catch(()=>{});}
